...
export default function NowPlaying() {
  ...
  const lastTrack = useRef<NowPlayingData | null>(null)

  useEffect(() => {
    if (data?.title && data?.artist) {
      lastTrack.current = data
    }
  }, [data])

  const displayData = data?.isPlaying ? data : lastTrack.current
  const isPaused = !!displayData && !data?.isPlaying && !!data
  ...
  useEffect(() => {
    ...
    // Keep showing the last known progress when paused
    setLiveProgress(progressRef.current.progress)
  }, [data?.isPlaying, data?.progress, displayData])
  ...
  return (
    <div
      className={`w-full max-w-[280px] p-4 rounded-2xl border shadow-lg transition-opacity duration-500 
        bg-white dark:bg-neutral-900 border-neutral-200 dark:border-white/10 opacity-100`}
    >
      <div className="flex items-center justify-between mb-3.5">
        <div className="flex items-center gap-1.5">
          <motion.span
            animate={{
              opacity: data?.isPlaying ? [1, 0.25, 1] : 1,
              scale: data?.isPlaying ? [1, 1.1, 1] : 1,
            }}
            transition={
              data?.isPlaying
          ...
          <span className="text-[10px] font-semibold tracking-widest uppercase text-neutral-400 dark:text-white/45">
            {data?.isPlaying ? 'Listening to' : 'Was listening to'}
          </span>
        </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-end gap-[2px] mt-2 h-3.5">
            {heights.map((h, i) => (
              <div
                key={i}
                style={{ height: data?.isPlaying ? h : 4 }}
                className={`w-[3px] rounded-sm transition-all duration-300 bg-green-500 `}
              />
            ))}
          </div>
        </div>
      ...
      <div className="mt-3.5">
        <div className="h-[3px] bg-neutral-200 dark:bg-white/[0.08] rounded-full overflow-hidden">
          <motion.div
            ...
            animate={{ width: data?.isPlaying ? `${pct}%` : '100%' }}
          />
        </div>

        <div className="flex justify-between mt-1.5">
          <span className="text-[10px] text-neutral-400 dark:text-white/30">
            {data?.isPlaying ? fmt(liveProgress) : 'Paused'}
          </span>
          <span className="text-[10px] text-neutral-400 dark:text-white/30">
