...
import { Settings, Shield, Key, CheckCircle2, Loader2, Palette, Save, Sparkles } from 'lucide-react'
...
const PRESET_THEMES: { name: string; colors: GithubTheme }[] = [
  {
    name: 'Classic Green',
    colors: {
      level0: '#161b22',
...
export default function AdminSettingsPage() {
  ...
  const handleSaveTheme = (e: React.FormEvent) => {
    ...
    startThemeTransition(async () => {
      ...
      if (res.success) {
        toast.success('GitHub contribution graph theme updated!')
      } else {
      ...
      <div className="flex items-center justify-between border-b border-zinc-800/80 pb-5">
        <div>
          ...
          <p className="text-xs text-zinc-400 font-mono pt-1">
            Manage your CMS credentials, security settings, and customize the GitHub contribution graph theme.
          </p>
        </div>
      </div>
      ...
      <div className="p-6 rounded-2xl border border-zinc-800/80 bg-zinc-900/60 backdrop-blur-md space-y-6 font-mono text-xs">
        <div className="flex items-center justify-between border-b border-zinc-800 pb-4">
          <h2 className="text-sm font-bold text-white uppercase tracking-wider flex items-center gap-2">
            ...
            <span>GitHub Graph Theme Customization</span>
          </h2>
          <span className="text-[10px] text-zinc-500">Live Preview & Presets</span>
        </div>
                ...
                <div className="flex gap-0.5">
                  {([0, 1, 2, 3, 4] as const).map((lvl) => (
                    <div
                      ...
                      style={{ backgroundColor: preset.colors[`level${lvl}`] }}
                    />
                  ))}
                </div>
            ...
            <div key={lvlKey} className="p-3 rounded-xl border border-zinc-800 bg-zinc-950 space-y-2">
              <div className="flex items-center justify-between">
                ...
                <div
                  ...
                  style={{ backgroundColor: themeColors[lvlKey] }}
                />
              </div>
              <div className="flex items-center gap-2">
                <input
                  type="color"
                  value={themeColors[lvlKey]}
                  onChange={(e) =>
                    setThemeColors((prev) => ({
                      ...prev,
        ...
        <div className="p-4 rounded-xl border border-zinc-800 bg-zinc-950 space-y-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-semibold text-zinc-300">Live Mini-Preview</span>
          </div>
          <div className="flex gap-[3px] overflow-hidden p-2 bg-zinc-900/50 rounded-lg border border-zinc-800/80 justify-center">
            {samplePattern.map((column, colIdx) => (
              <div key={colIdx} className="flex flex-col gap-[3px]">
                {column.map((levelVal, rowIdx) => (
                  <div
                    key={rowIdx}
                    className="w-3 h-3 rounded-[2px] border border-zinc-800/40"
                    style={{
                      backgroundColor: themeColors[`level${levelVal as 0 | 1 | 2 | 3 | 4}`],
                    }}
                  />
                ))}
              </div>
